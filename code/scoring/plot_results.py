...
model = MODELS[3] #0 for U-Net  or 1 for GCN
...
with open("/home/ctm/Desktop/GitLab/deep-image-segmentation-for-breast-contour-detection/code/Wilson_Files_Data/mask_test_221.pickle",'rb') as fp:  
        mask_test = pickle.load(fp)

gt_masks = np.concatenate((mask_train, mask_test))
gt_masks = gt_masks[test_indices_list[fold]]

print('Plotting results for model: ', model)
for i in range(X_original.shape[0]):
#for i in range(len(test_indices_list[fold])):
    plt.axis('off')
    #Show image
    plt.imshow(X_original[i])
    #plt.imshow(gt_masks[i], cmap='gray')
    #Plot predictions
    # plt.plot(resized_preds[i][0:34:2], resized_preds[i][1:35:2], 'b')
    # plt.plot(resized_preds[i][66:33:-2], resized_preds[i][67:34:-2], 'b')
    # plt.plot(resized_preds[i][0], resized_preds[i][1], 'bo')
    # plt.plot(resized_preds[i][32], resized_preds[i][33], 'bo')
    # plt.plot(resized_preds[i][66], resized_preds[i][67], 'bo')
    # plt.plot(resized_preds[i][34], resized_preds[i][35], 'bo')
    # plt.plot(resized_preds[i][70:74:2], resized_preds[i][71:75:2], 'bo')
    #Plot ground-truth
    # plt.plot(y_original[i][0:34:2], y_original[i][1:35:2], 'r')
    # plt.plot(y_original[i][66:33:-2], y_original[i][67:34:-2], 'r')
    # plt.plot(y_original[i][0], y_original[i][1], 'ro')
    # plt.plot(y_original[i][32], y_original[i][33], 'ro')
    # plt.plot(y_original[i][66], y_original[i][67], 'ro')
    # plt.plot(y_original[i][34], y_original[i][35], 'ro')
    # plt.plot(y_original[i][70:74:2], y_original[i][71:75:2], 'ro')
    plt.savefig(str(i), bbox_inches='tight', pad_inches=0.0)
    #plt.savefig(model+str(i), bbox_inches='tight', pad_inches=0.0)
    plt.show()